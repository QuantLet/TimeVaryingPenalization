...
## ![qlogo](https://github.com/QuantLet/Styleguide-and-Validation-procedure/blob/master/pictures/qloqo.png) **TVPactiveset**
...
plot(out_q$lambda.bic[, 1], type = "l",  col = alpha("darkblue", 0.05), axes = FALSE, 
     ...
     ylim = c(min(out_q$lambda.bic), max(out_q$lambda.bic)))
...
par(mar = c(3, 5, 1, 1))
plot(out_q$act.set[, 1], type = "l", col = alpha("darkblue", 0.05), axes = FALSE, xlab = "", 
     frame = TRUE, cex.main = 1.5, ylab = "q", xlim = c(-(w + 10), (n.obs - w + 10)), 
     ylim = c(min(out_q$act.set), max(out_q$act.set)))
axis(1, at = c(-w, n.cp - w, n.obs - w), labels = c("0", paste(expression("t ="), n.cp), n.obs),
     cex.axis = 1.2)
axis(2, cex.axis = 1.2)
abline(v = (n.cp - w), lty = 3)
...
plot(out_q$mean.k, type = "l",  col = "red3", axes = FALSE, 
     xlab = "Year", frame = TRUE, cex.main = 1.5, 
     ylab = expression(paste("Average ", kappa, " of X'X")),
